...
| `src/orchestrator/event-queue.ts` | Per-session FIFO for events posted to a `running` session. Queue drains between turns without flipping status to `idle`. |
...
| `src/runtime/gateway-ws.ts` | Operator-role WebSocket client to each container's OpenClaw gateway. Backs cancel/steer/patch/approvalResolve. Uses `client.id = "openclaw-tui"` (load-bearing — see the comment in `gateway-ws.ts`). |
| `src/runtime/parent-token.ts` | `ParentTokenMinter` — HMAC-SHA256 tokens for subagent delegation. One minter per orchestrator process; secret regenerates on restart. |
...
2. `AgentRouter.runEvent` — if session is `idle`: `beginRun` → fire-and-forget `executeInBackground`. If `running`: enqueue on `SessionEventQueue`.
3. `executeInBackground` → `pool.acquireForSession({sessionId, agentId, spawnOptions})` — three sources checked in order: active, warm (for this agentId), fresh spawn.
